...
function addTemplateField(field={output_name:"",target_type:"string",transforms:[],required:false,lookup_required:false}){
  const wrapper=document.createElement("div");wrapper.className="template-field fields";
  wrapper.innerHTML=`<label class="grow">Output name<input data-template="output_name" value="${esc(field.output_name)}"></label><label>Target type<select data-template="target_type">${["string","int","decimal","float","bool","date","datetime"].map(t=>`<option ${t===field.target_type?"selected":""}>${t}</option>`).join("")}</select></label><label>Ordered transforms<input data-template="transforms" placeholder="trim, empty_to_null" value="${esc(field.transforms.join(", "))}"></label><label>Required<input data-template="required" type="checkbox" ${field.required?"checked":""}></label><label>Max length<input data-template="max_length" type="number" min="1" value="${esc(field.max_length??"")}"></label><label>Lookup required<input data-template="lookup_required" type="checkbox" ${field.lookup_required?"checked":""}></label><button data-remove-target>Remove target</button>`;
...
function renderBindings(){
  ...
  $("template-bindings").hidden=false;updateBindingState();
}
