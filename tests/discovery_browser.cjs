...
(async()=>{
  ...
  try {
    ...
    const mappingState=()=>page.locator("#columns [data-field]").evaluateAll(inputs=>JSON.stringify(inputs.map(input=>[input.dataset.field,input.type==="checkbox"?input.checked:input.value])));
    const mappingBefore=await mappingState();
    ...
    assert.equal(await mappingState(),mappingBefore);
    ...
    await page.getByRole("button",{name:"Processed Preview (100 rows)",exact:true}).click();
    await page.getByRole("status").filter({hasText:"Choose 1–4096 output columns"}).waitFor();
    ...
    await page.getByRole("button",{name:"Read Source Sample",exact:true}).click();
    await waiting;
    await page.getByLabel("Encoding",{exact:true}).selectOption("utf-8");
    release();
